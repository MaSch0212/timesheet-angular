        ...
        this.settingsService.getSettings().subscribe(x => {
            setTimeout(() => {
                ...
                this.entryForm.entry = new TimeSheetEntry({
                    start: Helpers.setTimeFromString(
                        new Date(),
                        Helpers.getTimeStringFromHours(x.defaultStart)
                    ),
                    end: Helpers.setTimeFromString(
                        new Date(),
                        Helpers.getTimeStringFromHours(x.defaultEnd)
                    ),
                    breaks: !x.insertDefaultBreak
                        ...
                        : [
                              new Break({
                                  start: Helpers.setTimeFromString(
                                      new Date(),
                                      Helpers.getTimeStringFromHours(
                                          x.defaultBreakStart
                                      )
                                  ),
                                  end: Helpers.setTimeFromString(
