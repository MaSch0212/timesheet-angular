...
import { CopyApiKeyDialogComponent, CopyApiKeyDialogData } from '../copy-api-key-dialog/copy-api-key-dialog.component';
  ...
  onCreateClicked() {
    ...
    this.settingsService.createApiKey(this.keyName).subscribe(
      ...
      (error: HttpErrorResponse) => {
        this.isLoading = false;
        this.errorMsg = error.error ? error.error : `${error.status} (${error.statusText})`;
      }
    );
  }
}
