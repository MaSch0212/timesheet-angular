...
export class ApiKeysComponent implements OnInit {
  ...
  constructor(private dialog: MatDialog, private settingsService: SettingsService, private snackbar: MatSnackBar) {}
  ...
  onDeleteApiKey(index: number) {
    const dialogRef = this.dialog.open(DeleteWarningDialogComponent, {
      data: <DeleteWarningDialogData>{
        ...
        message: `Do you really want to delete the API Key "${this.apiKeys[index].name}"?`,
      },
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      this.settingsService.deleteApiKey(this.apiKeys[index].id).subscribe(
        ...
        (error: HttpErrorResponse) => {
          this.snackbar.open(`Failed to delete API Key${error.error ? ': ' + error.error : ''}`);
        }
      );
    });
  }
