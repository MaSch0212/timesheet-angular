...
export class TimeSheetComponent extends BaseComponent
    ...
    constructor(
        ...
        this.subscriptionManager.add(
            this.menuService.refreshRequested$.subscribe(() =>
                this.reloadData()
            )
        );
        this.subscriptionManager.add(
    ...
    onSave(
        old: TimeSheetEntry,
        current: TimeSheetEntry,
        row: TimeSheetEntryRowComponent
    ): void {
        if (current == null) {
            return;
        }
    ...
    onDelete(entry: TimeSheetEntry): void {
        const dialogRef = this.dialog.open(DeleteWarningDialogComponent, {
            data: <DeleteWarningDialogData>{
                title: 'Delete entry',
                message: `Do you really want to delete the entry starting on ${formatDate(
                    entry.start,
                    'short',
                    this.currentLocale
                )}?`
    ...
    reloadData(): void {
        ...
        this.scrollElementSelector$
            .pipe(first())
            .subscribe(x => document.querySelector(x).scrollTo(0, 0));
    ...
    addSpaceBetween(entry1: TimeSheetEntry, entry2: TimeSheetEntry): boolean {
        return (
            ...
            new Date(entry1.start.valueOf()).setHours(0, 0, 0, 0) -
            new Date(entry2.start.valueOf()).setHours(0, 0, 0, 0) >=
            7 * 24 * 60 * 60 * 1000
        );
    }
    ...
    private addEntry(entry: TimeSheetEntry): void {
        ...
        Helpers.insertIntoArray(
            group.entries,
            entry,
            x => x.start.getTime() < entry.start.getTime()
        );
    }

    private addGroup(year: number, month: number) {
        let result: ITimeSheetEntryGroup = new List<ITimeSheetEntryGroup>(
            this.entries
        ).SingleOrDefault(
        ...
        if (result == null) {
            ...
            Helpers.insertIntoArray(
                this.entries,
                result,
                x => x.date.getTime() < result.date.getTime()
            );
        }
        return result;
    }
    ...
    private removeEntry(entry: TimeSheetEntry): void {
        const group = new List<ITimeSheetEntryGroup>(
            this.entries
        ).SingleOrDefault(
            x =>
                x.date.getFullYear() === entry.start.getFullYear() &&
                x.date.getMonth() === entry.start.getMonth()
        );
        if (group != null) {
    ...
    private loadNextEntries(
        isReload: boolean = false,
        keepScroll: boolean = false
    ): void {
        if (
            (this.hasReachedEnd && !isReload) ||
            (this.isLoading && !isReload) ||
            this.isReloading
        ) {
            return;
        }
                ...
                .pipe(
                    map(newEntries => {
                        ...
                        newEntries.forEach(element => {
                            if (
                                lastGroup === null ||
                                lastGroup.date.getMonth() !==
                                element.start.getMonth() ||
                                lastGroup.date.getFullYear() !==
