...
import { NgForm, UntypedFormGroup, UntypedFormControl, Validators, UntypedFormArray } from '@angular/forms';
...
export class TimeSheetEntryFormComponent implements OnInit {
  ...
  onAddBreak() {
    (<UntypedFormArray>this.entryForm.get('breaks')).push(this.createBreakGroup(null));
  }

  getCurrentEntry(): TimeSheetEntry {
    if (this.entryForm.invalid) {
      this.errorMessage = 'Not all fields have been filled in correctly yet.';
      return null;
    }
    ...
    return new TimeSheetEntry({
      ...
      end: this.entryForm.value.end ? Helpers.setTimeFromString(date, this.entryForm.value.end) : null,
      breaks: this.entryForm.value.breaks.map(
        (b) =>
          new Break({
            id: b.id,
            start: Helpers.setTimeFromString(date, b.start),
            end: b.end ? Helpers.setTimeFromString(date, b.end) : null,
          })
      ),
  ...
  private createForm(entry: TimeSheetEntry) {
    ...
    this.entryForm = new UntypedFormGroup({
      ...
      date: new UntypedFormControl(entry ? Helpers.getDateString(entry.start) : null, Validators.required),
      start: new UntypedFormControl(entry ? Helpers.getTimeString(entry.start) : null, Validators.required),
      end: new UntypedFormControl(entry && entry.end ? Helpers.getTimeString(entry.end) : null),
      breaks: breaks,
      targetHours: new UntypedFormControl(entry ? entry.targetHours : null, Validators.required),
    });
  }
  ...
  private createBreakGroup(b: Break): UntypedFormGroup {
    return new UntypedFormGroup({
      ...
      start: new UntypedFormControl(b ? Helpers.getTimeString(b.start) : null, Validators.required),
      end: new UntypedFormControl(b && b.end ? Helpers.getTimeString(b.end) : null),
    });
  }
}
