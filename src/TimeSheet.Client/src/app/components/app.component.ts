...
export class AppComponent extends BaseComponent implements OnInit {
  ...
  isRefreshVisible$: Observable<boolean> = this.menuService.refreshButtonDisplayed$;
    ...
    this.subscriptionManager.add(
      fromEvent(document, 'visibilitychange').subscribe(() => {
        if (document.visibilityState === 'visible') {
          this.authGuard.canActivate(this.router.routerState.snapshot.root.firstChild, this.router.routerState.snapshot).subscribe();
        }
      })
    );
  }
