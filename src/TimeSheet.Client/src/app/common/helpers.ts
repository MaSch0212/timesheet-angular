export namespace Helpers {
  export function setTimeFromString(date: Date, time: string): Date {
    ...
    if (split.length === 1) {
      ...
      result.setHours(parseInt(split[0], 10), parseInt(split[1], 10), 0, 0);
    }
    return result;
  }
  ...
  export function setDateFromDate(date: Date, dateToSet: Date): Date {
    ...
    if (dateToSet == null || dateToSet.getTime() === 0) {
      ...
      result.setFullYear(dateToSet.getFullYear(), dateToSet.getMonth(), dateToSet.getDate());
    }
    return result;
  }
   ...
   */
  export function insertIntoArray<T>(array: Array<T>, element: T, funcAddBefore: (item: T) => boolean): void {
    let i: number;
  ...
  export function getTimeStringFromHours(value: number, useFullTimeFormat: boolean = false): string {
    ...
    return `${value < 0 && m > 0 ? '-' : ''}${h < 10 && useFullTimeFormat ? `0${h}` : `${h}`}:${m < 10 ? `0${m}` : `${m}`}`;
  }
